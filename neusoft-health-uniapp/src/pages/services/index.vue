      ...
      <view class="service-grid">
        <view class="svc-card" v-for="s in aiServices" :key="s.page" @click="navTo(s.page)">
          <view :class="['svc-icon-wrap', s.color]">
            <text class="svc-icon">{{ s.icon }}</text>
          </view>
...
const aiServices = [
  { name: 'AI健康咨询', page: 'consult', icon: '💬', desc: '智能对话问诊，症状分析', badge: '热门', badgeType: 'hot', color: 'bg-purple' },
  { name: '健康助手', page: 'health-assistant', icon: '🌐', desc: '症状自查、每日健康', badge: '热门', badgeType: 'hot', color: 'bg-blue' },
  { name: 'AI报告解读', page: 'health-report', icon: '📄', desc: '上传报告，智能分析', badge: '新功能', badgeType: 'new', color: 'bg-gold' },
  { name: '健康搜索', page: 'search', icon: '🔍', desc: '搜索症状、药品、知识', color: 'bg-green' },
]

const healthServices = [
  { name: '会员中心', page: 'member', icon: '👑', desc: '订阅会员、查看权益', color: 'bg-gold' },
  { name: '我的订单', page: 'order', icon: '📦', desc: '订单管理、退款申请', color: 'bg-red' },
  { name: '支付结算', page: 'payment', icon: '💳', desc: '扫码支付、订单结算', color: 'bg-blue' },
  { name: '健康档案', page: 'health-record', icon: '📋', desc: '个人健康档案管理', color: 'bg-green' },
  { name: '药品查询', page: 'medicine-search', icon: '💊', desc: '分类筛选、药品详情', color: 'bg-purple' },
  { name: '常见问题', page: 'faq', icon: '❓', desc: 'FAQ分类、快速解答', color: 'bg-teal' },
  { name: '消息通知', page: 'notification', icon: '🔔', desc: '系统/会员/咨询通知', color: 'bg-red' },
]

const quickLinks = [
  { text: '咨询历史', page: 'consult-history', icon: '🕐' },
  { text: '我的收藏', page: 'collections', icon: '❤️' },
  { text: '健康资讯', page: 'news', icon: '📰' },
]
